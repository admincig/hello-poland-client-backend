...
public class SightService extends ServiceSuperclass {
	...
	public Sight get(Long id) {
		Sight bo = em.createQuery("from Sight sight where sight.id=:sightId", Sight.class).setParameter("sightId", id)
				.getSingleResult();
		// fetch events
	...
	public List<Sight> getActive() {
		return em.createQuery("from Sight sight where sight.active=true order by sight.id desc", Sight.class)
				.getResultList();
	}
	...
	public Sight uploadMainImage(Long id, byte[] icon) {
		...
		var image = imageService.validateAndStoreImageCollector(is, "jpeg", null);
		Sight bo = get(id);

...
class PostgreSQLFullTextSearchRankStringFunction implements SQLFunction {
  ...
  public String render(Type firstArgumentType, List args, SessionFactoryImplementor factory) {
    ...
    if (ftsConfig != null) {
      return "ts_rank(to_tsvector(" + ftsConfig + ", " + field + "), plainto_tsquery(" + ftsConfig
          + ", " + value + "))";
    } else {
      return "ts_rank(to_tsvector(" + field + "), plainto_tsquery(" + value + "))";
    }
  }
